...
def _lib_books():
    st.markdown('<div style="padding:0 0.75rem">', unsafe_allow_html=True)
...
def _ch_main(bid,nav):
    ...
    with ci: st.markdown('<div style="background:#f9f7ff;border:1px solid #e9d5ff;border-radius:10px;padding:0.5rem 1rem;color:#7c3aed;font-size:0.85rem;line-height:1.5">💡 <b>Markdown</b> — **negrito** *itálico* ## Título &gt; Citação &nbsp;·&nbsp; Auto-save 45s</div>',unsafe_allow_html=True)
    with cp:
...
def _char_card(ch,bid):
    cid=ch["id"]
    photo_html=(f'<img src="{img_b64(ch["photo"])}" style="width:72px;height:72px;object-fit:cover;border-radius:50%;border:2px solid #6366f1">' if ch.get("photo") else '<div style="width:72px;height:72px;border-radius:50%;background:linear-gradient(135deg,#ede9ff,#c4b5fd);display:flex;align-items:center;justify-content:center;font-size:1.8rem">👤</div>')
...
def page_brain_dump():
    ...
    for d in dumps:
        ...
        if st.session_state.get(f"ed_bd_{did}"):
            ...
            border="border-left:4px solid #f59e0b;" if is_pinned else ""
